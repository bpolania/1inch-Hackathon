    ...
    try {
        ...
        try {
            ...
            if (!fundingResponse.data.status.confirmed) {
                console.log('   ⚠️  Bitcoin funding not yet confirmed, but proceeding...');
            }
        } catch (btcError) {
            console.log(`   ⚠️  Could not verify Bitcoin status: ${btcError.message}`);
        }
        ...
        try {
            ...
            console.log(`   ⚠️  Claim transaction creation: ${claimError.message}`);
