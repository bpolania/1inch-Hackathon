...
async function main() {
    ...
    if (!fs.existsSync('deployments-local.json')) {
        ...
        logInfo('Please run: npm run deploy:local');
        return;
    }
    ...
    // Create proper CosmWasm execution parameters using contract helper
    ...
    // Validate parameters through registry
    const validationResult = await registry.validateOrderParams(NEUTRON_TESTNET_ID, chainParams, ethers.parseUnits("1", 6));
